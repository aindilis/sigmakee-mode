...
(color coding as I did with the gtksourceview file - one color for comments, one for strings, one for variables, one for the seven logical operators (and, or, not, exists, forall, =>, <=>))
(paren balancing (easy since Emacs does this already))
(autocompletion - if you had a way to specify a file format for a list of terms and relations, I could write a function for Sigma to export such a file)
